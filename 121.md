...
executed automatically. Remember the "rootsum" Fortran program a while
...
OMProotsum.o : OMProotsum.f90
        $(F90) $(OPT) $(OMP) -c OMProotsum.f90

OMProotsum : OMProotsum.o
        $(F90) -o OMProotsum $(OMP) OMProotsum.o
...
If we are starting with the code (OMProotsum.f90), the submission script OMProotsum.sh, and this makefile, all we have to do is type

~~~~
$ make run
~~~~
~~~~
gfortran -O3 -fopenmp -c OMProotsum.f90
gfortran -o OMProotsum -fopenmp OMProotsum.o
echo 1234567890 > rootsum.in
OMP_NUM_THREADS=1 time -p ./OMProotsum < rootsum.in
...
sys 0.00
OMP_NUM_THREADS=8 time -p ./OMProotsum < rootsum.in
  Result =   28918862541603.332
real 0.52
user 3.93
sys 0.01
~~~~

Everything else happens automatically. If we want to submit the test run to the cluster we say

~~~~
hasch@caclogin02$ make submit
gfortran -O3 -fopenmp -c OMProotsum.f90
gfortran -o OMProotsum -fopenmp OMProotsum.o
echo 1234567890 > rootsum.in
sbatch OMProotsum.sh
Submitted batch job 4249294
~~~~

and instead of the runs happening on the workup node and the ouptut going to the screen, it gets submitted, and the output goes into the files STD.out and STD.err.
...
A word about the use of variables like "F90" and "OMP" in the
makefile. These are used instead of the actual terms "gfortran" and
