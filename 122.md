...
> The sum-of-squareroots example is very simple but it uses a common technique if very little or no communication is required during the execution of the workload on the processes: Broadcast/Reduce. We will encounter it a few more times. As an assignment, use the same method to compute an error function by numerical integration. 
